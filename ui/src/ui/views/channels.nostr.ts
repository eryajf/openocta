...
export function renderNostrCard(params: {
    ...
    return html`
      <div class="account-card">
        ...
        <div class="status-list account-card-status">
          ...
          <div>
            <span class="label">${t("channelPublicKey")}</span>
    ...
    return html`
      <div style="margin-top: 16px; padding: 12px; background: var(--bg-secondary); border-radius: 8px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
          <div style="font-weight: 500;">${t("nostrProfile")}</div>
          ${
            summaryConfigured
              ? html`
                <button
          ...
          hasAnyProfileData
            ? html`
              <div class="status-list">
                ...
                ${name ? html`<div><span class="label">${t("nostrName")}</span><span>${name}</span></div>` : nothing}
                ${
                  displayName
                    ? html`<div><span class="label">${t("nostrDisplayName")}</span><span>${displayName}</span></div>`
                    : nothing
                }
                ${
                  about
                    ? html`<div><span class="label">${t("nostrAbout")}</span><span style="max-width: 300px; overflow: hidden; text-overflow: ellipsis;">${about}</span></div>`
                    : nothing
                }
                ${nip05 ? html`<div><span class="label">${t("nostrNip05")}</span><span>${nip05}</span></div>` : nothing}
              </div>
            `
            : html`
  ...
  return html`
    <div class="card">
      <div class="card-title">${t("channelNostr")}</div>
      <div class="card-sub">${t("channelNostrSub")}</div>
      ${accountCountLabel}
        ...
        hasMultipleAccounts
          ...
          : html`
            <div class="status-list" style="margin-top: 16px;">
              <div>
                <span class="label">${t("channelConfigured")}</span>
                <span>${summaryConfigured ? t("commonYes") : t("commonNo")}</span>
              </div>
              <div>
                <span class="label">${t("channelRunning")}</span>
                <span>${summaryRunning ? t("commonYes") : t("commonNo")}</span>
              </div>
              <div>
                <span class="label">${t("channelPublicKey")}</span>
              ...
              <div>
                ...
                <span>${summaryLastStartAt ? formatAgo(summaryLastStartAt) : t("commonNA")}</span>
              </div>
            </div>
          `
      }
